    ...
    if request.POST:
        ...
        if user:
            if user[0].password == password:
                ...
                response.set_cookie("o_userid", user[0].id, 604800)
                response.set_cookie("o_username", user[0].username, 604800)
                response.set_cookie("o_password", user[0].password, 604800)
                return response
            else:
...
def logout(request):
    if request.POST:
        response = HttpResponse('success')
        response.delete_cookie("o_userid")
        response.delete_cookie("o_username")
        response.delete_cookie("o_password")
        return response
    else:
